...
class QAgentConfig:
    ...
    def __init__(self, gamma :float = 0.8, alpha:float = 0.1, epsilon :float =0.9, render :bool =False,
                 ...
                 tab_full_state_space : bool = False, a_mu0 : float = 0.0, a_lambda0 : float = 0.2,
                 a_alpha0 : float = 0.2, a_beta0 : float = 0.2, num_mixture_samples : int = 10,
                 d_mu0 : float = 0.0, d_lambda0 : float = 0.2, d_alpha0 : float = 0.2, d_beta0 : float = 0.2):
        """
        ...
        self.d_mu0 = d_mu0
        self.d_lambda0 = d_lambda0
        self.d_alpha0 = d_alpha0
        self.d_beta0 = d_beta0
    ...
    def to_str(self) -> str:
        ...
        return "Hyperparameters: gamma:{0},alpha:{1},epsilon:{2},render:{3},eval_sleep:{4}," \
               ...
               "checkpoint_freq:{21},random_seed:{22},eval_epsilon:{23},tab_full_state_space:{24}," \
               "a_mu0:{25},a_lambda0:{26},a_beta0:{27},d_mu0:{28},d_lambda0:{29},d_beta0:{30}," \
               "num_mixture_samples:{31}".format(
            ...
            self.random_seed, self.eval_epsilon, self.tab_full_state_space, self.a_mu0, self.a_lambda0,
            self.a_alpha0, self.a_beta0, self.d_mu0, self.d_lambda0, self.d_alpha0, self.d_beta0)
    ...
    def to_csv(self, file_path: str) -> None:
        ...
        with open(file_path, "w") as f:
            ...
            writer.writerow(["a_mu0", str(self.a_mu0)])
            writer.writerow(["a_beta0", str(self.a_beta0)])
            writer.writerow(["a_alpha0", str(self.a_alpha0)])
            writer.writerow(["a_lambda0", str(self.a_lambda0)])
            writer.writerow(["d_mu0", str(self.d_mu0)])
            writer.writerow(["d_beta0", str(self.d_beta0)])
            writer.writerow(["d_alpha0", str(self.d_alpha0)])
            writer.writerow(["d_lambda0", str(self.d_lambda0)])
            writer.writerow(["num_mixture_samples", str(self.num_mixture_samples)])
    ...
    def hparams_dict(self):
        ...
        hparams["a_mu0"] = self.a_mu0
        hparams["a_alpha0"] = self.a_alpha0
        hparams["a_beta0"] = self.a_beta0
        hparams["a_lambda0"] = self.a_lambda0
        hparams["d_mu0"] = self.d_mu0
        hparams["d_alpha0"] = self.d_alpha0
        hparams["d_beta0"] = self.d_beta0
        hparams["d_lambda0"] = self.d_lambda0
        hparams["num_mixture_samples"] = self.num_mixture_samples
        if self.dqn_config is not None:
